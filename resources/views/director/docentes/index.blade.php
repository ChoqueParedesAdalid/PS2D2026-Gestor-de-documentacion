...
<head>
    ...
    <title>Gestión de Docentes - DocGest Univalle</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .stat-card:nth-child(1) h3 { color: var(--status-info); }
        .stat-card:nth-child(2) h3 { color: var(--status-info); }
        .stat-card:nth-child(3) h3 { color: var(--status-approved); }
        .stat-card:nth-child(4) h3 { color: var(--status-purple); }
        
        .nav-tabs { border-bottom: 1px solid var(--border-soft); margin-bottom: 20px; }
        .nav-tabs .nav-link { color: var(--text-muted); border: none; padding: 12px 20px; font-weight: 500; transition: all 0.2s ease; }
        .nav-tabs .nav-link:hover { color: var(--text-primary); background: var(--hover-subtle); }
        .nav-tabs .nav-link.active { color: var(--text-primary); background: var(--bg-secondary); border-bottom: 2px solid var(--red-active); }
        ...
        .badge-docente { background: rgba(86, 204, 242, 0.15); color: var(--status-info); padding: 4px 12px; border-radius: 20px; font-size: 0.75rem; font-weight: 600; }
        .badge-tutor { background: rgba(46, 204, 113, 0.15); color: var(--status-approved); padding: 4px 12px; border-radius: 20px; font-size: 0.75rem; font-weight: 600; }
        .badge-tribunal { background: rgba(155, 89, 182, 0.15); color: var(--status-purple); padding: 4px 12px; border-radius: 20px; font-size: 0.75rem; font-weight: 600; }
...
<body>
    ...
    <nav class="top-navbar">
        <div class="navbar-left">
            ...
            <div class="navbar-title">Gestión de Docentes</div>
        </div>
    ...
    <main class="main-content"><div class="content-wrapper">
        ...
        <div class="page-header">
            <div><h2>Docentes Registrados</h2><p>Gestiona los docentes, tutores y tribunales del sistema</p></div>
        </div>

        <div class="stats-grid mb-4">
            <div class="stat-card"><h3>{{ $stats['total'] ?? 0 }}</h3><p>Total Docentes</p></div>
            <div class="stat-card"><h3>{{ $stats['docentes_carga'] ?? 0 }}</h3><p>Docentes a Cargo</p></div>
            <div class="stat-card"><h3>{{ $stats['tutores'] ?? 0 }}</h3><p>Tutores</p></div>
            <div class="stat-card"><h3>{{ $stats['tribunales'] ?? 0 }}</h3><p>Tribunales</p></div>
        </div>

        <ul class="nav nav-tabs" id="docentesTabs">
            <li class="nav-item"><a class="nav-link active" data-bs-toggle="tab" href="#docentes-carga">Docentes a Cargo</a></li>
            <li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#tutores">Tutores</a></li>
            <li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#tribunales">Tribunales</a></li>
        </ul>

        <div class="tab-content">
            <div class="tab-pane fade show active" id="docentes-carga">
                <div class="section-card"><div class="section-header"><h3><i class="fas fa-chalkboard-teacher" style="color: var(--status-info);"></i>Docentes a Cargo</h3></div><div class="section-body"><div class="table-responsive"><table class="table"><thead><tr><th>Nombre</th><th>Email</th><th>CI</th><th>Teléfono</th><th>Paralelos</th><th>Acciones</th></tr></thead><tbody>
                    @forelse($docentesCarga ?? [] as $docente)
                        <tr>
                            <td>{{ $docente->nombres }} {{ $docente->apellidos }}</td>
                            <td style="color: var(--text-secondary);">{{ $docente->email_institucional }}</td>
                            <td>{{ $docente->ci }}</td>
                            <td style="color: var(--text-secondary);">{{ $docente->telefono ?? 'N/A' }}</td>
                            <td><span class="badge-docente">0</span></td>
                            <td><div class="d-flex gap-2"><button class="btn-action"><i class="fas fa-eye"></i></button><button class="btn-action edit"><i class="fas fa-edit"></i></button></div></td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="text-center py-4" style="color: var(--text-muted);">No hay docentes a cargo registrados</td></tr>
                    @endforelse
                </tbody></table></div></div></div>
            </div>

            <div class="tab-pane fade" id="tutores">
                <div class="section-card"><div class="section-header"><h3><i class="fas fa-user-check" style="color: var(--status-approved);"></i>Tutores</h3></div><div class="section-body"><div class="table-responsive"><table class="table"><thead><tr><th>Nombre</th><th>Email</th><th>CI</th><th>Teléfono</th><th>Tutorados</th><th>Acciones</th></tr></thead><tbody>
                    @forelse($tutores ?? [] as $tutor)
                        <tr>
                            <td>{{ $tutor->nombres }} {{ $tutor->apellidos }}</td>
                            <td style="color: var(--text-secondary);">{{ $tutor->email_institucional }}</td>
                            <td>{{ $tutor->ci }}</td>
                            <td style="color: var(--text-secondary);">{{ $tutor->telefono ?? 'N/A' }}</td>
                            <td><span class="badge-tutor">0/5</span></td>
                            <td><div class="d-flex gap-2"><button class="btn-action"><i class="fas fa-eye"></i></button><button class="btn-action edit"><i class="fas fa-edit"></i></button></div></td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="text-center py-4" style="color: var(--text-muted);">No hay tutores registrados</td></tr>
                    @endforelse
                </tbody></table></div></div></div>
            </div>

            <div class="tab-pane fade" id="tribunales">
                <div class="section-card"><div class="section-header"><h3><i class="fas fa-gavel" style="color: var(--status-purple);"></i>Tribunales</h3></div><div class="section-body"><div class="table-responsive"><table class="table"><thead><tr><th>Nombre</th><th>Email</th><th>CI</th><th>Teléfono</th><th>Evaluaciones</th><th>Acciones</th></tr></thead><tbody>
                    @forelse($tribunales ?? [] as $tribunal)
                        <tr>
                            <td>{{ $tribunal->nombres }} {{ $tribunal->apellidos }}</td>
                            <td style="color: var(--text-secondary);">{{ $tribunal->email_institucional }}</td>
                            <td>{{ $tribunal->ci }}</td>
                            <td style="color: var(--text-secondary);">{{ $tribunal->telefono ?? 'N/A' }}</td>
                            <td><span class="badge-tribunal">0</span></td>
                            <td><div class="d-flex gap-2"><button class="btn-action"><i class="fas fa-eye"></i></button><button class="btn-action edit"><i class="fas fa-edit"></i></button></div></td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="text-center py-4" style="color: var(--text-muted);">No hay tribunales registrados</td></tr>
                    @endforelse
                </tbody></table></div></div></div>
            </div>
        </div>
