...
<head>
    ...
    <title>Detalle de Paralelo - DocGest Univalle</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .info-card { background: var(--bg-secondary); border: 1px solid var(--border-soft); border-radius: 12px; padding: 20px; }
        .info-card h4 { font-size: 0.85rem; color: var(--text-muted); margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.5px; }
        .info-card p { font-size: 1.1rem; font-weight: 600; color: var(--text-primary); margin: 0; }
        .info-card.highlight { border-left: 4px solid var(--red-active); }
        
        .stats-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 24px; }
        .stat-card { background: var(--bg-secondary); border: 1px solid var(--border-soft); border-radius: 12px; padding: 20px; text-align: center; }
        .stat-card h3 { font-size: 2rem; font-weight: 700; margin-bottom: 4px; color: var(--status-info); }
        .stat-card p { margin: 0; font-size: 0.85rem; color: var(--text-muted); }
        
        .section-card { background: var(--bg-secondary); border: 1px solid var(--border-soft); border-radius: 12px; overflow: hidden; margin-bottom: 24px; }
        .section-header { padding: 16px 20px; border-bottom: 1px solid var(--border-soft); display: flex; align-items: center; justify-content: space-between; }
        .section-header h3 { font-size: 1rem; font-weight: 600; margin: 0; color: var(--text-primary); }
        .section-body { padding: 24px; }
        
        .btn-action { background: var(--red-button); color: var(--text-primary); border: none; padding: 6px 16px; border-radius: 6px; text-decoration: none; font-size: 0.85rem; transition: all 0.2s ease; display: inline-flex; align-items: center; gap: 6px; }
        .btn-action:hover { background: var(--red-active); }
        .btn-secondary { background: transparent; color: var(--text-secondary); border: 1px solid var(--border-soft); padding: 6px 16px; border-radius: 6px; text-decoration: none; font-size: 0.85rem; transition: all 0.2s ease; display: inline-flex; align-items: center; gap: 6px; }
        .btn-secondary:hover { background: var(--hover-subtle); color: var(--text-primary); }
        
        .table { color: var(--text-primary); margin-bottom: 0; }
        .table thead th { background: rgba(255,255,255,0.03); border-bottom: 1px solid var(--border-soft); color: var(--text-muted); font-size: 0.75rem; text-transform: uppercase; padding: 12px 16px; font-weight: 600; border-top: none; }
...
<body>
    ...
    <nav class="top-navbar">
        <div class="navbar-left">
            ...
            <div class="navbar-title">Paralelo {{ $paralelo->letra }}</div>
        </div>
    ...
    <main class="main-content"><div class="content-wrapper">
        ...
        <div class="page-header">
            <div><h2>Paralelo {{ $paralelo->letra }} - {{ $paralelo->materia?->nombre ?? 'Sin materia' }}</h2></div>
            <div class="d-flex gap-2">
                <a href="{{ route('director.paralelos.estudiantes', $paralelo->id) }}" class="btn-action"><i class="fas fa-users"></i> Ver Estudiantes</a>
                <a href="{{ route('director.paralelos.editar', $paralelo->id) }}" class="btn-secondary"><i class="fas fa-edit"></i> Editar</a>
        ...
        <div class="info-grid">
            <div class="info-card highlight">
                ...
                <p>{{ $paralelo->gestion?->nombre ?? 'N/A' }}</p>
            </div>
            <div class="info-card">
                <h4>Materia</h4>
                <p>{{ $paralelo->materia?->nombre ?? 'Sin materia' }}</p>
            </div>
            <div class="info-card">
                <h4>Docente a Cargo</h4>
                <p>{{ $paralelo->docenteCargo ? $paralelo->docenteCargo->nombres . ' ' . $paralelo->docenteCargo->apellidos : 'Sin asignar' }}</p>
            </div>
            <div class="info-card">
                <h4>Período</h4>
                <p>{{ $paralelo->fecha_inicio ? \Carbon\Carbon::parse($paralelo->fecha_inicio)->format('d/m/Y') : 'N/A' }} - {{ $paralelo->fecha_fin ? \Carbon\Carbon::parse($paralelo->fecha_fin)->format('d/m/Y') : 'N/A' }}</p>
            </div>
        </div>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                <h3 style="color: var(--status-approved);">{{ ucfirst($paralelo->estado ?? 'activo') }}</h3>
                <p>Estado</p>
            </div>
        </div>
        ...
        <div class="section-card">
            <div class="section-header">
                <h3><i class="fas fa-info-circle" style="color: var(--red-active);"></i> Información Detallada</h3>
            </div>
            <div class="section-body">
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <strong style="color: var(--text-muted);">ID del Paralelo:</strong>
                        <p class="mb-0" style="color: var(--text-primary);">{{ $paralelo->id }}</p>
                    </div>
                    <div class="col-md-6 mb-3">
                        <strong style="color: var(--text-muted);">Fecha de Creación:</strong>
                        <p class="mb-0" style="color: var(--text-primary);">{{ $paralelo->created_at ? $paralelo->created_at->format('d/m/Y H:i') : 'N/A' }}</p>
                    </div>
                    <div class="col-md-6 mb-3">
                        <strong style="color: var(--text-muted);">Última Actualización:</strong>
                        <p class="mb-0" style="color: var(--text-primary);">{{ $paralelo->updated_at ? $paralelo->updated_at->format('d/m/Y H:i') : 'N/A' }}</p>
                    </div>
                    <div class="col-md-6 mb-3">
                        <strong style="color: var(--text-muted);">Estado:</strong>
                        <p class="mb-0"><span class="badge" style="background: rgba(46, 204, 113, 0.15); color: var(--status-approved); padding: 4px 12px; border-radius: 20px; font-size: 0.75rem;">{{ ucfirst($paralelo->estado) }}</span></p>
                    </div>
                </div>
            </div>
        </div>
