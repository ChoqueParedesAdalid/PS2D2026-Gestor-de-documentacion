...
<head>
    ...
    <title>Editar Paralelo - DocGest Univalle</title>
...
<body>
    ...
    <nav class="top-navbar">
        <div class="navbar-left">
            ...
            <div class="navbar-title">Editar Paralelo</div>
        </div>
    ...
    <main class="main-content"><div class="content-wrapper">
        ...
        <div class="page-header">
            <div><h2>Editar Paralelo {{ $paralelo->letra }}</h2><p>Modifica la información del paralelo</p></div>
            <a href="{{ route('director.paralelos') }}" class="btn-back"><i class="fas fa-arrow-left"></i>Volver</a>
        </div>

        <div class="form-card">
            <div class="form-header"><i class="fas fa-edit"></i><h3>Información del Paralelo</h3></div>
            <div class="form-body">
                <form action="{{ route('director.paralelos.actualizar', $paralelo->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <select name="gestion_id" class="form-select" required>
                            ...
                            @foreach($gestiones as $gestion)
                                <option value="{{ $gestion->id }}" {{ $paralelo->gestion_id == $gestion->id ? 'selected' : '' }}>{{ $gestion->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <select name="materia_id" class="form-select" required>
                                ...
                                @foreach($materias as $materia)
                                    <option value="{{ $materia->id }}" {{ $paralelo->materia_id == $materia->id ? 'selected' : '' }}>{{ $materia->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 mb-4">
                            ...
                            <select name="letra" class="form-select" required>
                                @foreach(['A','B','C','D','E','F','G','H'] as $letra)
                                    <option value="{{ $letra }}" {{ $paralelo->letra == $letra ? 'selected' : '' }}>{{ $letra }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="docente_cargo_id" class="form-select">
                            ...
                            @foreach($docentes as $docente)
                                <option value="{{ $docente->id }}" {{ $paralelo->docente_cargo_id == $docente->id ? 'selected' : '' }}>{{ $docente->nombres }} {{ $docente->apellidos }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-4">
                            ...
                            <input type="date" name="fecha_inicio" class="form-control" value="{{ old('fecha_inicio', $paralelo->fecha_inicio?->format('Y-m-d')) }}" required>
                        </div>
                        <div class="col-md-6 mb-4">
                            <label class="form-label">Fecha de Fin <span class="required">*</span></label>
                            <input type="date" name="fecha_fin" class="form-control" value="{{ old('fecha_fin', $paralelo->fecha_fin?->format('Y-m-d')) }}" required>
                        </div>
                    </div>

                    <div class="form-actions">
                        <a href="{{ route('director.paralelos') }}" class="btn-cancel">Cancelar</a>
                        <button type="submit" class="btn-save"><i class="fas fa-save"></i>Actualizar Paralelo</button>
                    </div>
                </form>
            </div>
        </div>
