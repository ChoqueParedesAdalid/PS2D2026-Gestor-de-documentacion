...
<head>
    ...
    <title>@yield('title', 'DocGest - Docente a Cargo')</title>
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside 
            ...
            <div class="navbar-brand">
                <div class="brand-logos">
                    <img src="{{ asset('img/logo_docgest.png') }}" alt="DocGest">
            ...
            <nav class="mt-6 px-2">
                <a href="{{ route('docente.dashboard') }}" 
                   class="sidebar-link flex items-center px-4 py-3 text-white rounded-lg {{ request()->routeIs('docente.dashboard') ? 'active' : '' }}">
                    <i class="fas fa-home w-6"></i><span>Inicio</span>
                </a>
                <a href="{{ route('docente.estudiantes', ['materiaId' => request('materiaId') ?? 1]) }}" 
                   class="sidebar-link flex items-center px-4 py-3 text-white rounded-lg {{ request()->routeIs('docente.estudiantes') ? 'active' : '' }}">
                    <i class="fas fa-user-graduate w-6"></i><span>Estudiantes</span>
                </a>
                <a href="{{ route('docente.tareas', ['materiaId' => request('materiaId') ?? 1]) }}" 
                   class="sidebar-link flex items-center px-4 py-3 text-white rounded-lg {{ request()->routeIs('docente.tareas') ? 'active' : '' }}">
                    <i class="fas fa-tasks w-6"></i><span>Tareas</span>
                </a>
            </nav>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            <header class="bg-black shadow-sm border-b border-gray-800 flex-shrink-0">
                <div class="flex items-center justify-between px-6 py-4">
                    <div class="flex items-center">
                        ...
                        <h2 class="text-xl font-semibold text-white">@yield('page-title', 'Panel Docente')</h2>
                    </div>
                    <div class="flex items-center space-x-4">
                        <div class="hidden sm:flex items-center px-3 py-1 bg-green-900 bg-opacity-50 rounded-lg">
                            <i class="fas fa-chalkboard-teacher text-green-400 mr-2"></i>
                            <span class="text-sm text-white">Docente a Cargo</span>
                        </div>
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" @click.away="open = false"
                                ...
                                <div class="text-right hidden sm:block">
                                    <p class="text-sm font-medium text-white">{{ auth()->user()->nombres ?? 'Docente' }}</p>
                                    <p class="text-xs text-gray-400">Docente a Cargo</p>
                                </div>
                                <div class="h-10 w-10 rounded-full bg-red-700 flex items-center justify-center text-white font-semibold hover:bg-red-600 transition cursor-pointer">
                                    {{ substr(auth()->user()->nombres ?? 'D', 0, 1) }}
                                </div>
                            </button>
            ...
            <main class="flex-1 overflow-x-hidden overflow-y-auto p-6">
                @if(session('success'))
                    <div class="bg-green-900 bg-opacity-70 border border-green-600 text-green-100 px-4 py-3 rounded mb-4">{{ session('success') }}</div>
                @endif
                @if(session('error'))
                    <div class="bg-red-900 bg-opacity-70 border border-red-600 text-red-100 px-4 py-3 rounded mb-4">{{ session('error') }}</div>
                @endif
                @yield('content')
            </main>
            ...
            <footer class="footer">
                <div class="footer-container">
                    <div class="footer-brand">
                        ...
                        <p>DocGest · <span>Universidad del Valle</span><br><span style="font-size: 0.75rem; color: #6b7280;">Sistema de Gestión Académica</span></p>
                    </div>
