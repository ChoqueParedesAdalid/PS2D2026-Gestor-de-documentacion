...
<head>
    ...
    <title>@yield('title', 'Docente a Cargo - DocGest Univalle')</title>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .stat-card{background:var(--bg-secondary);border:1px solid var(--border-soft);border-radius:12px;padding:20px;border-left:4px solid;display:flex;align-items:center;justify-content:space-between;animation:fadeInUp .5s ease-out;transition:all .3s ease}
    .stat-card:hover{transform:translateY(-5px);box-shadow:0 10px 30px rgba(139,26,26,.3)}
    ...
    @media(max-width:992px){.stats-grid{grid-template-columns:repeat(2,1fr)}.content-row{grid-template-columns:1fr}.sidebar{transform:translateX(-100%)}.sidebar.show{transform:translateX(0)}.top-navbar,.main-content,.footer{left:0}}
    </style>
    @stack('styles')
</head>
<body>
        ...
        <nav class="sidebar-menu">
            ...
            <div class="nav-item">
                <a href="{{ route('docente.estudiantes') }}" class="nav-link {{ request()->routeIs('docente.estudiantes*') ? 'active' : '' }}">
                    <i class="fas fa-users"></i><span>Gestión Estudiantes</span>
                </a>
            </div>
            <div class="nav-item">
                <a href="{{ route('docente.proyectos') }}" class="nav-link {{ request()->routeIs('docente.proyectos*') ? 'active' : '' }}">
                    <i class="fas fa-project-diagram"></i><span>Gestión Proyectos</span>
                </a>
            </div>
            <div class="nav-item">
                <a href="{{ route('docente.tareas') }}" class="nav-link {{ request()->routeIs('docente.tareas*') ? 'active' : '' }}">
    ...
    <nav class="top-navbar">
        ...
        <div class="navbar-right">
            <div class="director-badge">
                <i class="fas fa-chalkboard-teacher"></i>Docente a Cargo<span class="badge-active">Activo</span>
            </div>
            <div class="notification-bell"><i class="fas fa-bell"></i><span class="notification-badge">3</span></div>
            <div class="user-profile">
                <div class="user-info">
                    <div class="user-name">{{ Auth::user()->nombres ?? 'Docente' }}</div>
                    <div class="user-role">Docente a Cargo</div>
                </div>
