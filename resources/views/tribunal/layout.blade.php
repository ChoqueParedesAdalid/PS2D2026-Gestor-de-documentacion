...
<head>
    ...
    <title>@yield('title', 'DocGest - Tribunal')</title>
...
<body class="main-gradient min-h-screen" x-data="{ sidebarOpen: false }">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside 
            ...
            <nav class="mt-6 px-2">
                <a href="{{ route('tribunal.dashboard') }}" 
                   class="sidebar-link flex items-center px-4 py-3 text-white rounded-lg {{ request()->routeIs('tribunal.dashboard') ? 'active' : '' }}">
                    <i class="fas fa-home w-6"></i><span>Inicio</span>
                </a>
                <a href="{{ route('tribunal.estudiantes') }}" 
                   class="sidebar-link flex items-center px-4 py-3 text-white rounded-lg {{ request()->routeIs('tribunal.estudiantes') ? 'active' : '' }}">
                    <i class="fas fa-users w-6"></i><span>Estudiantes</span>
                </a>
                <a href="{{ route('tribunal.tareas') }}" 
                   class="sidebar-link flex items-center px-4 py-3 text-white rounded-lg {{ request()->routeIs('tribunal.tareas') || request()->routeIs('tribunal.tareas-ver') ? 'active' : '' }}">
                    <i class="fas fa-tasks w-6"></i><span>Tareas</span>
                </a>
                <a href="{{ route('tribunal.documentos') }}" 
                   class="sidebar-link flex items-center px-4 py-3 text-white rounded-lg {{ request()->routeIs('tribunal.documentos') ? 'active' : '' }}">
                    <i class="fas fa-file-alt w-6"></i><span>Documentos</span>
                </a>
            </nav>
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" @click.away="open = false"
                                ...
                                <div class="text-right hidden sm:block">
                                    <p class="text-sm font-medium text-white">{{ auth()->user()->nombres ?? 'Tribunal' }}</p>
                                    <p class="text-xs text-gray-400">Docente Tribunal</p>
                                </div>
                                <div class="h-10 w-10 rounded-full bg-red-700 flex items-center justify-center text-white font-semibold hover:bg-red-600 transition cursor-pointer">
                                    {{ substr(auth()->user()->nombres ?? 'T', 0, 1) }}
                                </div>
                            </button>
