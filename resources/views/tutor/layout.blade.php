...
<head>
    ...
    * {
    ...
    /* Badge "Pronto" para el botón Tribunal */
    .badge-pronto {
        font-size: 0.65rem;
...
<body class="main-gradient min-h-screen">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <!-- ============ SIDEBAR ============ -->
        <aside class="w-64 sidebar-gradient shadow-lg flex-shrink-0 hidden md:block relative">
            ...
            <nav class="mt-6 px-2">
                <a href="{{ route('tutor.dashboard') }}" 
                   class="sidebar-link flex items-center px-4 py-3 text-white rounded-lg {{ request()->routeIs('tutor.dashboard') ? 'active' : '' }}">
                    <i class="fas fa-home w-6"></i>
                    <span>Inicio</span>
                </a>
                
                <a href="{{ route('tutor.tutorados') }}" 
                   class="sidebar-link flex items-center px-4 py-3 text-white rounded-lg {{ request()->routeIs('tutor.tutorados') ? 'active' : '' }}">
                    <i class="fas fa-users w-6"></i>
                    <span>Tutorados</span>
                </a>
                
                <a href="{{ route('tutor.documentos') }}" 
                   class="sidebar-link flex items-center px-4 py-3 text-white rounded-lg {{ request()->routeIs('tutor.documentos') ? 'active' : '' }}">
                    <i class="fas fa-file-alt w-6"></i>
                    <span>Lista Documentos</span>
                </a>
                
                <a href="{{ route('tutor.revisar') }}" 
                   class="sidebar-link flex items-center px-4 py-3 text-white rounded-lg {{ request()->routeIs('tutor.revisar') ? 'active' : '' }}">
                    <i class="fas fa-tasks w-6"></i>
                    <span>Revisar Documentos</span>
                </a>
            </nav>
            
            <!-- BOTÓN CERRAR SESIÓN (fijo al fondo del sidebar) -->
            <div class="absolute bottom-4 left-4 right-4">
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" 
                            class="w-full flex items-center justify-center space-x-2 px-4 py-3 bg-red-900 bg-opacity-70 hover:bg-opacity-90 text-white rounded-lg transition border border-red-700 shadow-lg">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="bg-black shadow-sm border-b border-gray-800 flex-shrink-0">
                <div class="flex items-center justify-between px-6 py-4">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <!-- BOTÓN TRIBUNAL -->
                        @if(auth()->user() && auth()->user()->esTutor())
                        <a href="#" 
                           class="hidden sm:flex items-center space-x-2 px-3 py-2 bg-black bg-opacity-50 border border-red-700 rounded-lg text-white hover:bg-opacity-70 transition"
                           title="Vista de Tribunal (próximamente)"
                           onclick="return false;">
                            <i class="fas fa-gavel text-red-400"></i>
                            <span class="text-sm font-medium">Tribunal</span>
                        ...
                        <!-- Notificaciones -->
                        <div class="relative">
                            <button class="text-white hover:text-red-500 focus:outline-none" title="Notificaciones">
                        ...
                        <div class="flex items-center space-x-3">
                            <div class="text-right hidden sm:block">
                                <p class="text-sm font-medium text-white">{{ auth()->user()->nombres ?? 'Tutor' }}</p>
            ...
            <!-- ===== FOOTER (dentro del área de contenido principal) ===== -->
            <footer class="footer">
